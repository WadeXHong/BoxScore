package com.bardxhong.boxscore.modelhelper.task;
...
public class OnCreateDataBaseTask extends AsyncTask<Void, Void, Void> {
    ...
    protected Void doInBackground(Void... voids) {
        ...
        ArrayList<String> gameIdList = BoxScore.getGameInfoDbHelper().getGameHistoryList();
        HashMap hashMap = (HashMap) mDataSnapshot.getValue();
        
        for (String gameId : gameIdList) {
            if (!hashMap.containsKey(gameId)) {
                BoxScore.getGameInfoDbHelper().deleteGameInfo(null, gameId);
        ...
        for (DataSnapshot games : mDataSnapshot.child(Constants.FireBaseConstant.NODE_NAME_GAME_INFO).getChildren()) {
            ...
            // Db 沒有在抓
            int count = BoxScore.getGameInfoDbHelper().getSpecificInfo(gameId).getCount();
            if (count != 0) continue;
        ...
        for (DataSnapshot players : mDataSnapshot.child(Constants.FireBaseConstant.NODE_NAME_TEAM_PLAYER).getChildren()) {
            ...
            // Db 沒有在抓
            int count = BoxScore.getTeamDbHelper().getSpecificPlayer(playerId).getCount();
            if (count != 0) continue;
        ...
        for (DataSnapshot teams : mDataSnapshot.child(Constants.FireBaseConstant.NODE_NAME_TEAM_INFO).getChildren()) {
            ...
            // Db 沒有在抓
            int count = BoxScore.getTeamDbHelper().getSpecificTeamInfo(teamId).getCount();
            if (count != 0) continue;
